...
import ol_render_getVectorContext from 'ol-ext/util/getVectorContext';
...
var ol_interaction_SelectCluster = function (options) {
	...
	options.layers = [options.layer, this.overlayLayer_];

	this.debounceSelect = debounce(this.selectAllClusters, 50);

	this._layerFeatureAddedListener = options.layer.getSource().on('addfeature', () => this.layerFeatureAdded.call(this));
	...
	if (options.filter) {
		...
		//if (l===overlay && f.get("selectclusterlink")) return false;
		if (!l && (f.get('selectclusterlink') || f.get('selectclusterfeature'))) {
			return false;
		} else {
			return true;
		}
...
ol_interaction_SelectCluster.prototype.removeListeners = function () {
	this.debounceSelect.cancel();
	if (this._layerFeatureAddedListener) ol_Observable_unByKey(this._layerFeatureAddedListener);
	this._layerFeatureAddedListener = null;
};

ol_interaction_SelectCluster.prototype.layerFeatureAdded = function () {
	if (this.selectAll) {
		this.debounceSelect();
	}
};

ol_interaction_SelectCluster.prototype.selectAllClusters = function () {
	const self = this;
	this.selectAll = true;
	const features = this.layer.getSource().getFeatures();
	this.clear(true);
	features.forEach((f) => {
		self.selectCluster(f);
	});
};

ol_interaction_SelectCluster.prototype.deActivateSelectAllClusters = function () {
 ...
 * @api stable
 */
ol_interaction_SelectCluster.prototype.clear = function (force) {
	if (!this.selectAll || force) {
